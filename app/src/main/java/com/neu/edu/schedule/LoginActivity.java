...
public class LoginActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initEvent() {
        ...
        rememberPaw.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    @Override
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tvForgetPwd:// 忘记密码的单击事件
                Toast.makeText(getApplicationContext(), "忘了重新注册",
                        Toast.LENGTH_SHORT).show();
                break;
            case R.id.rule:// 忘记密码的单击事件
                Toast.makeText(
                        getApplicationContext(),
                        "学号为8位数，前四位为入学年份，后四位为个人编号 ",
            ...
            case R.id.btnLogin:// 登录按钮的单击事件
                ...
                if ("".equals(editUserName.getText().toString())
                    ...
                    Toast.makeText(getApplicationContext(), "学号或密码为空",
                            Toast.LENGTH_SHORT).show();
                } else if (editUserName.getText().toString().length() != 8) {
                    Toast.makeText(getApplicationContext(), "这位同志，学号长度必须为8位哦！",
                            Toast.LENGTH_SHORT).show();
                } else if (!util.isOK(editUserName.getText().toString())) {// 此时用户输入的长度一定是8，进行判断：是不是现在的大学生
